...
import { Position, TecackDataset } from "@tecack/shared";
...
export const codegen = defineCommand({
  ...
  run: async ({ args }) => {
    try {
      ...
      const code = codegenInternal(parsed, { ts, cjs, exportName });
...
const codegenInternal = (parsed: TecackDataset[], { ts, cjs, exportName }: CodegenOptions): string => {
  const asTs = ts && !cjs;
